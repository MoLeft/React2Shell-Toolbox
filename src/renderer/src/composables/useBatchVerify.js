...
const logger = createLogger('BatchVerify')
...
export function useBatchVerify(
  ...
  const scrollToVerifyingRow = async (rowIndex) => {
    try {
      ...
      if (!resultsBodyRef.value) {
        logger.debug('resultsBodyRef 不存在')
        return
      }

      const rows = resultsBodyRef.value.querySelectorAll('.results-table tbody tr')
      logger.debug(`找到 ${rows.length} 行，尝试滚动到第 ${rowIndex} 行`)
      ...
      if (rowIndex >= rows.length) {
        logger.debug(`行索引超出范围: ${rowIndex} >= ${rows.length}`)
        return
      }
      ...
      logger.debug('滚动命令已执行')
    } catch (error) {
      logger.error('滚动失败', error)
    }
  }
  ...
  const addToVulnHistory = async (url) => {
    try {
      logger.info('正在保存漏洞历史', { url })
      const result = await window.api.storage.addHistoryItem(url)
      if (result.success) {
        logger.success('漏洞历史保存成功')
      } else {
        logger.error('漏洞历史保存失败', result.error)
      }
    } catch (error) {
      logger.error('保存漏洞历史失败', error)
    }
  }
  ...
  const executeHijack = async (url, settings) => {
    try {
      ...
      if (hijackResult.success && hijackResult.data.is_vulnerable) {
        logger.success('劫持成功', { url })
        return true
      } else {
        logger.error('劫持失败', { url, error: hijackResult.error })
        return false
      }
    } catch (error) {
      logger.error('劫持错误', error)
      return false
    }
  }
  ...
  const executeBatchVerify = async (loadPageData) => {
    try {
      ...
      logger.info(`开始验证第 ${currentPageNum} 页`, {
        totalPages: totalPagesCount,
        cached: !!searchResultsCache.value[currentPageNum]
      })

      // 如果当前页没有缓存数据，尝试加载
      if (!searchResultsCache.value[currentPageNum]) {
        logger.info(`第 ${currentPageNum} 页无缓存，开始加载数据`)
        try {
          await loadPageData(currentPageNum)
          logger.success(`第 ${currentPageNum} 页数据加载完成`)
          ...
          while (
            ...
            logger.debug(`等待第 ${currentPageNum} 页数据写入缓存 (${retryCount + 1}/30)`)
            await new Promise((resolve) => setTimeout(resolve, 100))
            retryCount++
          }

          logger.debug(`第 ${currentPageNum} 页缓存检查完成`, {
            hasData: !!searchResultsCache.value[currentPageNum]
          })
        } catch (error) {
          logger.error(`加载第 ${currentPageNum} 页失败`, error)
          // 加载失败，停止验证
          clearHighlightedRow()
          return {
            success: false,
        ...
        if (
          ...
          logger.warn(`第 ${currentPageNum} 页加载后仍然没有数据，可能已到达最后`)
      ...
      logger.info(`第 ${currentPageNum} 页共有 ${pageResults.length} 条数据`)
      ...
      logger.info(`第 ${currentPageNum} 页有 ${pendingItems.length} 条待验证项`)

      // 如果当前页没有待验证的项，跳转到下一页或结束
      if (pendingItems.length === 0) {
        logger.info(`当前页 ${currentPageNum} 没有待验证项`)
        consecutiveEmptyPages.value++

        // 如果连续3页都没有待验证项，或者已经是最后一页，停止验证
        if (consecutiveEmptyPages.value >= 3 || currentPageNum >= totalPagesCount) {
          logger.info(`连续 ${consecutiveEmptyPages.value} 页没有待验证项或已到最后一页，验证结束`)
        ...
        if (currentPageNum < totalPagesCount && batchVerifying.value && !isChangingPage.value) {
          logger.info(`当前页无待验证项，跳转到下一页 ${currentPageNum + 1}`)
          ...
          try {
            ...
            logger.debug(`检查第 ${nextPage} 页缓存状态`, {
              cached: !!searchResultsCache.value[nextPage]
            })
            ...
            logger.debug(`递归调用验证第 ${nextPage} 页`)
      ...
      const verifyItem = async ({ item, index }) => {
        ...
        try {
          ...
          if (result.is_vulnerable && result.digest_content) {
            ...
            logger.info('检测到漏洞', { url: item.fullUrl })
            logger.debug('autoHijackEnabled', unref(autoHijackEnabled))
      ...
      if (currentPageNum < totalPagesCount && batchVerifying.value && !isChangingPage.value) {
        logger.info(`当前页 ${currentPageNum} 验证完成，跳转到下一页 ${currentPageNum + 1}`)
